...
Table.prototype.count = function(){
  ...
  var sql = "select COUNT(1) from " + this.name + " where " + args.where;
...
Table.prototype.where = function(){
  var args = ArgTypes.whereArgs(arguments);
  var sql = "select * from " + this.name + " where " + args.where;
...
Table.prototype.find = function(){
  ...
  var sql = "select " + cols + " from \"" + this.name + "\"" + where.where + order + limit + offset;
