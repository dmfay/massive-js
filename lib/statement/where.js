...
const getCondition = function (key) {
  ...
  var i = 0,
    ...
    isJSON = false,         // does this query traverse a JSON field?
    isJSONArray = false,    // does it specify a JSON array element?
  ...
  do {
    switch (char) {
      ...
      case ':':
        ...
        break;
      case '.':
        // json path traversal. ignore the first ., then just snap up the
        // entire path.
        if (!isJSON) {
          isJSON = true;
          buffer = parsed[parsed.push([]) - 1];
  ...
  if (isJSON) {
    const tokens = parsed.shift().split('.');

    if (tokens.length === 1) {
      quotedField = `${quotedField}->>'${tokens[0]}'`;
    } else {
