...
const Query = function (conditions = {}, options = {}, object = {}) {
  ...
  if (where.isPkSearch(conditions, options)) {
    if (typeof object.primaryKeyName === 'function') {
      if (_.isObject(conditions)) {
        this.where = where(conditions);
      } else {
        // table pk search
        this.where = where(_.object([[object.primaryKeyName(), conditions]]));
        this.single = true;
      }
    } else {
  ...
  } else if (_.isObject(conditions) && conditions.hasOwnProperty('conditions')) {
  ...
  } else if (_.isObject(conditions) && this.document) {
