...
Database.prototype.attach = function (entities) {
  ...
  return entities.map(entity => {
    ...
    if (!existing) {
      ...
      // determine whether we add entity's properties to the existing node on
      // the tree or replace the node with entity and apply the old existing
      ...
      // followed by Tables, which in turn define a superset of Queryable
      ...
      const winner = both.find(e => typeof e === 'function') ||
        both.find(e => e instanceof Table) ||
        both.find(e => e instanceof Queryable) ||
        entity;

      const loser = both.find(e => e !== winner);

      // determine the appropriate prototype for the new object based on the
      // types of both entities: Table > Queryable > Executable > Entity.

      const newProto = both.some(e => e instanceof Table) ? Table :
        both.some(e => e instanceof Queryable) ? Queryable :
        both.some(e => e instanceof Executable) ? Executable :
        Entity;

      // does this prototype define a method which would override an already-
      // attached function? then throw

      for (const prop in winner) {
        if (
          typeof winner[prop] === 'function' &&
 ...
 */
Database.prototype.detach = function (entity) {
  ...
  this.objects = _.reject(this.objects, function (element) {
  ...
  return this.$p.all(initPromises).then(() => {
    ...
    return this.$p.all([{
      ...
      return promises.concat(defn.loaders.map(loader => {
        ...
        // run the loader
        return require(`./loader/${loader}`)(this.instance, this.loader).then(loaded => { // eslint-disable-line global-require
 ...
 *
 * @return {Array} A list of view names.
 */
Database.prototype.listViews = function () {
  return this.objects.reduce((names, obj) => {
    if (obj instanceof Queryable && !(obj instanceof Table)) {
      names.push(obj.path);
    }
 ...
 */
Database.prototype.getObject = function (location) {
  if (location.startsWith('public.')) {
    location = location.substring(7);
  }

  return _.get(this, location);
};
