...
DB.prototype.query = function () {
  ...
  if (options.formatter) {
    promise = promise.then(result => {
      if (options.single) { return Promise.resolve(options.formatter.formatObject(result)); }
      else { return Promise.resolve(options.formatter.formatArray(result)); }
    });
  }
