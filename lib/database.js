...
Database.prototype.attach = function (entities) {
  ...
  return entities.map(entity => {
    ...
    if (!existing) {
      ...
      // types of _both_ entities: Table > Queryable > Executable > Entity.

      const newProto = _.cond([
        [() => both.some(e => e instanceof Table), _.constant(Table)],
        [() => both.some(e => e instanceof Queryable), _.constant(Queryable)],
...
Database.prototype.getObject = function (location) {
  if (location.startsWith(`${this.currentSchema}.`)) {
    location = location.substring(this.currentSchema.length + 1);
  }
