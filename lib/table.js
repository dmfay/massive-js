...
Table.prototype.insert = function (data, options) {
  if (!this.insertable) {
    ...
    return Promise.reject(new Error('Must provide data to insert'));
  }
 ...
 */
Table.prototype.update = function (criteria, changes, options = {}) {
  if (!this.insertable) {
    return Promise.reject(new Error(`${this.name} is not writable`));
...
Table.prototype.save = function (record, options = {}) {
  ...
  if (_.intersection(keys, this.pk).length === this.pk.length) {
    ...
    if (!criteria) {
      return Promise.reject(new Error(`Incomplete or invalid primary key criteria for $
{this.name}.save`));
    }
 ...
 * modified row.
 */
Table.prototype.updateDoc = function (criteria, changes, field = 'body') {
  const options = {
    document: field === 'body'
  };
  ...
  const sql = `UPDATE ${this.delimitedFullName} SET "${field}" = COALESCE("${field}", '{}'::jsonb) || $1 WHERE ${queryCriteria.conditions} RETURNING *;`;
