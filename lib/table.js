...
const isPkSearch = require('./query/is-pk-search');
...
Table.prototype.update = function(conditions, fields) {
  ...
  if (_.isUndefined(fields)) {
    ...
    conditions = _.fromPairs([[this.pk, fields[this.pk]]]);
  ...
  const sql = `UPDATE ONLY ${this.delimitedFullName} SET ${f.join(', ')} WHERE ${criteria.conditions} RETURNING *;`;
...
Table.prototype.destroy = function(args) {
  ...
  const sql = `DELETE FROM ONLY ${this.delimitedFullName} WHERE ${criteria.conditions} RETURNING *;`;
...
Table.prototype.modify = function(criteria, changes, field = 'body') {
  ...
  if (isPkSearch(criteria)) {
    ...
    queryCriteria = {
      conditions: `${this.pk} = $2`,
      params: criteria
    };

    options.single = true;
  } else {
    queryCriteria = where(criteria, 1, options.document ? 'docGenerator' : 'generator');
  }
