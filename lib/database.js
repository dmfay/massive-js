 ...
 * @param {String} loader.verUUID - If documentPkType is set to 'uuid', set which UUID version to use,
 ...
 *
 * @param {String} pkType - Primary key type, 'serial' or 'uuid' expected
 * @param {String} uuidV - The UUID variant/version to use; typicaly 'uuid_generate_v1' or 'uuid_generate_v4'
...
function getDefaultSQLforUUID (pkType, uuidV) {
  ...
  switch (uuidV) {
    ...
    case 'uuid_generate_v5':
      ...
      break;
    case 'uuid_nil':
      sqlDefault = 'DEFAULT uuid_nil()';
      break;
    case 'uuid_ns_dns':
      sqlDefault = 'DEFAULT uuid_ns_dns()';
      break;
    case 'uuid_ns_oid':
      sqlDefault = 'DEFAULT uuid_ns_oid()';
      break;
    case 'uuid_ns_url':
      sqlDefault = 'DEFAULT uuid_ns_url()';
      break;
    case 'uuid_ns_x500':
      sqlDefault = 'DEFAULT uuid_ns_x500()';
      break;
    default:
      sqlDefault = 'DEFAULT uuid_generate_v4()';
  }
...
Database.prototype.createDocumentTable = function (location) {
  ...
  const verUUID = this.loader.verUUID || 'uuid_generate_v4';
  const sqlDefault = getDefaultSQLforUUID(documentPkType, verUUID);
