...
Massive.prototype.saveDoc = co.wrap(function* (collection, doc) {
  let potentialTable = this.getTable(collection);
 ...
 */
Massive.prototype.getTable = function (path) {
  const splits = path.split(".");
  const tableName = splits.pop();
  const schemaName = splits.pop() || "public";

  return this.tables.find((table) => (table.schema == schemaName && table.name == tableName));
};
