...
exports.forTable = function(conditions, placeholderOffset) {
  ...
  _.each(conditions, function(value, key) {
    ...
    var mutator = operation.mutator || function (v) { return v; };
    var property;
    ...
    if (value === null) {
      ...
      operator = operator === '!=' || operator === '<>' ? 'IS NOT' : 'IS';
    } else if (!_.isArray(value) || operation.mutator) {
      //parameterize any remaining non-array values
      result.params.push(mutator(value));
    ...
    if (_.isArray(value)) {
      ...
      return _conditions.push(util.format('%s %s (%s)', property, operator == '!=' || operator == '<>' ? 'NOT IN' : 'IN', arrayConditions.join(', ')));
    } else {
      return _conditions.push(util.format('%s %s %s', property, operator, value));
    }
  });
...
exports.forDocument = function(conditions) {
  ...
  _.each(conditions, function(value, key) {
    ...
    if (_.isBoolean(value)) {
      return _conditions.push(util.format("(body ->> '%s')::boolean %s %s", property, operator, value));
    } else if(_.isDate(value)) {
      result.params.push(value);
      return _conditions.push(util.format("(body ->> '%s')::timestamp %s $%d", property, operator, result.params.length));
    } else if(_.isNumber(value)) {
      return _conditions.push(util.format("(body ->> '%s')::decimal %s %d", property, operator, value));
    }

    //anything non-array handling
    if (!_.isArray(value)) {
      result.params.push(value);
      return _conditions.push(util.format("(body ->> '%s') %s %s", property, operator, "$" + (result.params.length)));
    }
    ...
    _conditions.push(util.format("(body ->> '%s') %s (%s)", property, operator == '!=' || operator == '<>' ? 'NOT IN' : 'IN', arrayConditions.join(', ')));
  });
