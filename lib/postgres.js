  ...
  pg.connect(_connectionString, function(err,db){
    ...
    db.query(query.sql, query.params,function(err, results){
      if(err) callback(err,null);
        ...
        query.emit("executed");
        query.on("close", function(){console.log("Closing...");db.close()});
      }
    });
  });
};
...
var executeStreamed = function(query) {
  console.log("Executing streamed");
  pg.connect(_connectionString, function(err,db){
    ...
    dbQuery.on("close", function(){console.log("Closing...");db.close()});
  });
};

var executePassThrough= function(query) {
  console.log("Executing passthrough");
  pg.connect(_connectionString, function(err,db){
    if(err) throw err;
...
Client.prototype.execute = function(query, callback) {
  ...
  if(typeof(query) === "object" && typeof(callback) === "object"){
    ...
    executeWithCallback(query,callback);
  } else {
