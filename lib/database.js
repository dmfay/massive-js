 ...
 *
 * @function
 * @param {String} [tag] - Tag the transaction for external monitoring.
 * @param {Function} [func] - The transactional function.
 * @param {TransactionMode} [mode] - Override the default
 * {@link https://vitaly-t.github.io/pg-promise/txMode.TransactionMode.html|transaction mode}.
 */
Massive.prototype.transaction = function () {
  const args = Array.prototype.slice.call(arguments);

  if (_.last(args) instanceof this.pgp.txMode.TransactionMode) {
    args[args.length - 2].txMode = args.pop();
  }

  // TODO Queryable/Table/Executable need to use an override to db.query which
  // runs in the session initiated by pgp.tx. This is probably going to involve
  // proxying the db objects to swap in the override.
  //
  // It might be easiest/cleanest to restrict func to generators
  return this.instance.tx.apply(this.instance, args);
};
