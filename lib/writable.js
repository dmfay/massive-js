...
Writable.prototype.saveDocs = function (docs) {
  if (!_.isArray(docs)) {
    return this.db.instance.$config.promise.reject('Please pass in the documents for saving as an object.');
  }

  if (!docs.every(_.isObjectLike)) {
    return this.db.instance.$config.promise.reject('Please pass in valid documents. Include the primary key for an UPDATE.');
  }

  return this.db.instance.$config.promise.all(docs.map(this.saveDoc.bind(this)));
};
