...
exports.forTable = function(conditions, placeholderOffset) {
  ...
  _.each(conditions, function(value, key) {
    ...
    if (parts.length === 3) {
      property = util.format('"%s"%s\'%s\'', parts[0], parts[1], parts[2]);
      value = value.toString();   // make sure we pass in a string; postgres has trouble matching up operators otherwise
    } else {
