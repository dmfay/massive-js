...
exports.forDocument = function (conditions = {}, offset = 0, prefix = '\nWHERE ') {
  ...
  if (keys.length === 1 && /^id[^\w\d]?/.test(keys[0])) {
    // pk search (with possible operator), use forTable
    where = exports.forTable(conditions, offset, prefix);
    where.pkQuery = /^id\s*=?$/.test(keys[0]);
  } else {
