...
var MySQL = function(credentials){
  ...
  this.run = function(sql,params, callback) {
    this.execute(sql, params, function(err, results, db) {
...
this.dropTable = function(tableName) {
  return new Query("DROP TABLE IF EXISTS " + tableName + ";", [], self, new Table(tableName, "", self));
}
...
this.createTable = function(tableName, columns) {
  ...
  _sql+= _cols.join(",") + ");";
  return new Query(_sql, [], self, new Table(tableName, columns.id, self));
};
