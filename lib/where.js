...
exports.predicate = function (result, condition, value) {
  if (value === null) {
    ...
    value = "$" + (result.params.length + result.offset);
...
exports.docPredicate = function (result, condition, value, conditions) {
  ...
  if(_.isArray(value) && _.isObject(value[0])) {
    ...
    result.predicates.push(util.format("body @> $%s", result.params.length));
  }
  ...
  //not ideal, but it works nicely
    ...
    result.predicates.push(util.format("(body ->> '%s')::boolean %s %s", condition.field, condition.operator, value));
  } else if(_.isDate(value)) {
    result.params.push(value);
    result.predicates.push(util.format("(body ->> '%s')::timestamp %s $%d", condition.field, condition.operator, result.params.length));
  } else if(_.isNumber(value)) {
    result.predicates.push(util.format("(body ->> '%s')::decimal %s %d", condition.field, condition.operator, value));
  }

  //anything non-array handling
  else if (!_.isArray(value)) {
    result.params.push(value);
    result.predicates.push(util.format("(body ->> '%s') %s $%s", condition.field, condition.operator, result.params.length));
  } else {
    var arrayConditions = [];
    ...
    result.predicates.push(util.format("(body ->> '%s') %s (%s)", condition.field, condition.operator, arrayConditions.join(', ')));
  }
...
exports.forTable = function() {
  ...
  _.each(args.conditions, function(value, key) {
    var condition = exports.parseKey(key);

    if (condition.field === 'or') {
      if (!_.isArray(value)) { value = [value]; }

      var predicates = value.map(function (subgroup) {
        // generate the simple predicates for each key-value pair in each subgroup
        var subresult = _.reduce(subgroup, function (acc, subvalue, subkey) {
          var subcondition = exports.parseKey(subkey);

          var subresult = exports[args.predicate](acc, subcondition, subvalue, args.conditions);

          return acc;
        }, {params: [], predicates: [], offset: result.params.length});

        result.params = result.params.concat(subresult.params);
        // join (with AND) and encapsulate the simple predicates by group ("(x = $1 AND y = $2)")
        return util.format('(%s)', subresult.predicates.join(' AND '));
      });

      // join (with OR), encapsulate, and push the complex predicate ("(x = $1 AND y = $2) OR (z = $3)")
      result.predicates.push(util.format('(%s)', predicates.join(' OR ')));
    } else {
      result = exports[args.predicate](result, condition, value, args.conditions);
    }
  });
