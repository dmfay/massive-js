...
var Table = function(tableName, pk, _db) {
  ...
  this.find = function() {
    return new Query("SELECT * FROM " + this.name, [], this.db, this).parseArgs(arguments);
  };

  this.count = function(where) {
    return new Query("SELECT COUNT(1) FROM " + this.name, [], this.db, this).where(where);
  };

  this.destroy = function() {
    return new Query("DELETE FROM " + this.name, [], this.db, this).parseArgs(arguments);
  };
  ...
  this.insert = function(data) {
    ...
    return new Query(sql, _.flatten(parameters), this.db, this);
  };
  ...
  this.update = function(fields, where){
    ...
    return new Query(sql, parameters, this.db, this).where(where);
  };
};
