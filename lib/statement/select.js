...
Select.prototype.format = function () {
  const the_columns = this.fields;
  the_columns.forEach(function(v,i){
    const dot_index = v.indexOf('.');
    if (dot_index !== -1){
      const col_infos = v.split(' ');
      the_columns[i] = `"${col_infos[0].slice(0,dot_index)}"#>>'{${col_infos[0].slice(dot_index+1).replace('.',',')}}'`;
      if (!col_infos[1]){
        col_infos[1] = v.replace('.','_');
      }
      the_columns[i] += ' '+col_infos[1];
    }
  });
