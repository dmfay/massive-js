...
const Select = function (source, criteria = {}, options = {}) {
  ...
  if (this.pageLength) {
    if (!_.isPlainObject(criteria)) {
      this.error = 'Keyset paging with pageLength requires a criteria object';
    } else if (!Object.prototype.hasOwnProperty.call(options, 'order')) {
      ...
      this.params = options.order.map(o => o.last);

      // TODO use order-by proper for body/casts/dir/etc
      const paginationColumns = options.order.map(o => `"${o.field || o.expr}"`).join(',');
      const placeholders = options.order.map((o, idx) => `$${idx + 1}`).join(',');
...
Select.prototype.format = function () {
  ...
  sql += `${this.source} WHERE `;

  if (this.pagination) { sql += `${this.pagination} AND `; }

  sql += `${this.where.conditions} ${this.order}`;
