 ...
 *
 * @param {Object|Array} entity - New Entity or list of Entities to add to the
 * instance.
 * @return {Array} All added entities.
 */
Database.prototype.attach = function (entity) {
  const entities = _.castArray(entity);
  ...
  return entities.map(obj => {
    ...
    if (obj instanceof Executable) {
      executor = function () { return obj.invoke(...arguments); };
    }

    // do we already have something at this location?
    const existing = _.get(this, obj.path);

    // TODO perform the merge

    _.set(this, obj.path, executor || obj);
