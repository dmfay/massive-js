 ...
 * (default ./db).
 * @param {String} loader.createDocumentTablePkDataType - Override the Massive createDocumentTable serial data type
 * set for the primary key id, e.g. uuid
...
Database.prototype.createDocumentTable = function (location) {
  ...
  const createDocumentTablePkDataType = this.loader.createDocumentTablePkDataType || 'serial';
  const queryFile = createDocumentTablePkDataType === 'uuid' ? 'document-table-uuid.sql' : 'document-table.sql';

  return this.query(this.loader.queryFiles[queryFile], {
    schema: schemaName,
    table: tableName,
    index: indexName
  }).then(() =>
